            ...
            for (let name in global.features) {
                ...
                if (opts && match && m) {
                    ...
                    if (plugin.error >= 5) {
                        ...
                        m.reply("*[ ⚠️ ]* Fitur ini di matikan karena sedang mengalami *bug/error*")
                        continue;
                    }
                    ...
                    if (!db.data.users[m.sender].limit > 100 & db.data.users[m.sender].limit < 1) {
...
Notifikasi telah di kirim ke nomor Owner bot jika fitur.ini masih Error selama *5 kali* maka akan otomatis mematikan fitur ini !`);
    ...
    async participantsUpdate({
        ...
        switch (action) {
            ...
            case "remove":
                if (action === "add") {
                    metadata.participants.push(...participants.map(sender => ({
                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        let reasn = check ? `di ${action === "add" ? "tambahkan" : "keluarkan" } oleh @${author.split("@")[0]}` : `${action === "add" ? "Bergabung ke dalam" : "Keluar dari" } group`
                        ...
                        try {
                            ...
                            text =
                                action === "add" ?
                                db.data.chats[id].welcome.replace("%member", "@" + user.split("@")[0]).replace("%subject", gpname).replace("%reason", reasn).replace("%time", moment.tz("Asia/Jakarta").format("HH:mm")) :
                                global.db.data.chats[id].leave.replace("%member", "@" + user.split("@")[0]).replace("%subject", gpname).replace("%reason", reasn).replace("%time", moment.tz("Asia/Jakarta").format("HH:mm"))
                            let wel = pp;
                            let lea = pp;
