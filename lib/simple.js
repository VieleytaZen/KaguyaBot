...
exports.makeWASocket = (connectionOptions, store, options = {}) => {
  let kgy = (global.opts["legacy"] ? makeWALegacySocket : makeWASocket)(connectionOptions, );
  kgy.decodeJid = (jid) => {
    if (!jid) return jid;
    if (/:\d+@/gi.test(jid)) {
  ...
  (kgy.sendFile = async (jid, path, filename = "", caption = "", quoted, ptt = false, options = {}, ) => {
    ...
    try {
      ...
      if (!m) m = await kgy.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...options, ...ephemeral }, );
      return m;
    }
  });
  kgy.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
    ...
    await kgy.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted, ...ephemeral }, );
    return buffer;
  };
...
`.trim();
  ...
  (kgy.reply = (jid, text = "", quoted, options) => {
    return Buffer.isBuffer(text) ? kgy.sendFile(jid, text, "file", "", quoted, false, options) : kgy.sendMessage(jid, { ...options, text, mentions: kgy.parseMention(text) }, {
      quoted,
      ...options,
   ...
   * @returns
   */
  kgy.sendGroupV4Invite = async (jid, participant, inviteCode, inviteExpiration, groupName = "unknown subject", caption = "Invitation to join my WhatsApp group", options = {}, ) => {
   ...
   * @param {Object} options
   */
  kgy.copyNForward = async (jid, message, forwardingScore = true, options = {}, ) => {
  ...
  kgy.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true, ) => {
  ...
  kgy.getName = async (jid = "", withoutContact = false) => {
    ...
    if (jid.endsWith("@g.us")) return new Promise(async (resolve) => {
      ...
      jid,
      vname: "WhatsApp",
    } : areJidsSameUser(jid, kgy.user.id) ? kgy.user : {};
    return (
      (withoutContact ? "" : v.name) || v.subject || v.vname || v.notify || v.verifiedName || PhoneNumber("+" + jid.replace("@s.whatsapp.net", "")).getNumber("international", ));
  };
  /**
  ...
  kgy.processMessageStubType = async (m) => {
    ...
    const chat = kgy.decodeJid(m.key.remoteJid || m.message?.senderKeyDistributionMessage?.groupId || "", );
  ...
  kgy.sendPoll = async (jid, name = "", optiPoll, options) => {
    ...
    return kgy.relayMessage(jid, { pollCreationMessage: pollMessage }, { ...options }, );
  };
  /*
   * Set auto Bio
   */
...
exports.smsg = (kgy, m, hasParent) => {
  ...
  if (m.key) {
    ...
    m.chat = kgy.decodeJid(m.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || "", );
    m.isGroup = m.chat.endsWith("@g.us");
    m.sender = kgy.decodeJid(
      (m.key.fromMe && kgy.user.id) || m.participant || m.key.participant || m.chat || "", );
  ...
  if (m.message) {
    ...
    if (m.mtype == "protocolMessage" && m.msg.key) {
      ...
      m.msg.key.fromMe = kgy.decodeJid(m.msg.key.participant) === kgy.decodeJid(kgy.user.id);
      if (!m.msg.key.fromMe && m.msg.key.remoteJid === kgy.decodeJid(kgy.user.id)) m.msg.key.remoteJid = m.sender;
    }
    m.text = m.msg.text || m.msg.caption || m.msg.contentText || m.msg || "";
    if (typeof m.text !== "string") {
      if (
        ["protocolMessage", "messageContextInfo", "stickerMessage", "audioMessage", "senderKeyDistributionMessage", ].includes(m.mtype)) m.text = "";
    ...
    if (m.quoted) {
      ...
      m.quoted.chat = kgy.decodeJid(m.msg.contextInfo.remoteJid || m.chat || m.sender, );
      ...
      if (m.quoted.url || m.quoted.directPath) m.quoted.download = (saveToFile = false) => kgy.downloadM(m.quoted, m.quoted.mtype.replace(/message/i, ""), saveToFile, );
      m.quoted.reply = (text, chatId, options) => kgy.reply(chatId ? chatId : m.chat, text, vM, options);
      m.quoted.copy = () => exports.smsg(kgy, M.fromObject(M.toObject(vM)));
      m.quoted.forward = (jid, forceForward = false) => kgy.forwardMessage(jid, vM, forceForward);
      m.quoted.copyNForward = (jid, forceForward = true, options = {}) => kgy.copyNForward(jid, vM, forceForward, options);
      m.quoted.cMod = (jid, text = "", sender = m.quoted.sender, options = {}, ) => kgy.cMod(jid, vM, text, sender, options);
      m.quoted.delete = () => kgy.sendMessage(m.quoted.chat, { delete: vM.key });
    }
  }
  m.name = m.pushName || kgy.getName(m.sender);
  if (m.msg && m.msg.url) m.download = (saveToFile = false) => kgy.downloadM(m.msg, m.mtype.replace(/message/i, ""), saveToFile);
  m.copy = () => exports.smsg(kgy, M.fromObject(M.toObject(m)));
  m.forward = (jid = m.chat, forceForward = false) => kgy.copyNForward(jid, m, forceForward, options);
  ...
  m.copyNForward = (jid = m.chat, forceForward = true, options = {}) => kgy.copyNForward(jid, m, forceForward, options);
  m.cMod = (jid, text = "", sender = m.sender, options = {}) => kgy.cMod(jid, m, text, sender, options);
  m.delete = () => kgy.sendMessage(m.chat, { delete: m.key });
  try {
    if (m.msg && m.mtype == "protocolMessage") kgy.ev.emit("message.delete", m.msg.key);
  } catch (e) {
    console.error(e);
  }
  return m;
