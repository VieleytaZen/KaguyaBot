...
module.exports = {
    ...
    run: async (m, {
        ...
        if (/image/.test(mime)) {
            ...
            let data = await Func.fetchJson(`https://api.yanzbotz.my.id/api/ai/gemini-image?url=${url}&query=${text}&apiKey=${api.yanz}`);
            m.reply(`*[ GEMINI ]*\n${data.result}`);
        } else if (/video/.test(mime)) {
            let url = await uploader(await q.download());
            let data = await Func.fetchJson(`https://api.yanzbotz.my.id/api/ai/gemini-video?url=${url}&query=${text}&apiKey=${api.yanz}`);
            m.reply(`*[ GEMINI ]*\n${data.result}`);
        } else {
            let data = await Func.fetchJson(`https://api.yanzbotz.my.id/api/ai/gemini?query=${text}&apiKey=${api.yanz}`);
            m.reply(`*[ GEMINI ]*\n${data.result}`);
        }
    },
    ...
    description: "Dapatkan hasil jawaban lebih akurat dengan Google Ai, dapat menjawab pertanyaan mulai dari berupa text, gambar dan video !"
}