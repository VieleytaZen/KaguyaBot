    ...
    run: async (m, {
        ...
        Object.keys(features)
            ...
            .forEach(plugin => {
                ...
                const tagsArray = Array.isArray(feature.category) ? feature.category.filter(tag => tag) : [];
                const helpArray = Array.isArray(feature.help) ? feature.help.filter(help => help) : [feature.help].filter(help => help);
                ...
                if (feature.command) {
                    helpArray.forEach(helpItem => {
                        ...
                        commandMapping[helpItem] = Object.fromEntries(
                            Object.entries(commandMapping[helpItem]).filter(([_, value]) => value)
                        );
                    });
                }
            });
        ...
        let help = Object.values(features)
            ...
            .map(plugin => {
                const result = {
                    help: Array.isArray(plugin.help) ? plugin.help.filter(help => help) : [plugin.help].filter(help => help),
                    tags: Array.isArray(plugin.category) ? plugin.category.filter(tag => tag) : [plugin.category].filter(tag => tag),
...
└───────────────⭓`;
        ...
        const cap = Func.Styles(`Hai`) + ` @${m.sender.split("@")[0]} ` + Func.Styles(`Perkenalkan nama saya Kaguya, saya adalah asisten pintar yang siap membantu Anda dengan fitur-fitur yang saya punya ^^
...
*• Total Fitur :* ${Object.values(features).filter((v) => v.help).map((v) => v.help).flat(1).length}
...
${allTagsAndHelp}`);
        ...
        if (text && Object.keys(commandMapping).includes(text)) {
            ...
            let hasil = Func.Styles(`┌─⭓「 *${text.split("").join(" ").toUpperCase()}* 」
